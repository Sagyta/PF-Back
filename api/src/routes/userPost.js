...
router.post('/', async (req, res, next) => {
  ...
  if (await User.findOne({ where: { dni: dni } }))
    return res.json({ error: 'DNI ya existente en nuestra base de datos' });
  if (await User.findOne({ where: { username: username } }))
    return res.json({ error: 'Usuario ya existente en nuestra base de datos' });
  ...
  try {
    ...
    await sendMail(
      email,
      'Confirma tu email en Club Deportivo Henry',
      template
    );
    await newUser.save();
    res.json({
  ...
  try {
    ...
    if (code !== user.code) {
      return res
        .status(404)
        .redirect(
          'https://img.freepik.com/vector-gratis/concepto-fallo-tecnico-landing-page_52683-10996.jpg?t=st=1656446573~exp=1656447173~hmac=ad04d6e9d78368c16673ae8df038eaf78586d38eed5b748ef373a1eae52e84b3&w=740'
        );
    }
